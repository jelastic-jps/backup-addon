...
function backup(){
    ...
    RESTIC_PASSWORD=${ENV_NAME} restic -q -r /opt/backup snapshots || RESTIC_PASSWORD=${ENV_NAME} restic init -r /opt/backup
    ...
    RESTIC_PASSWORD=${ENV_NAME} restic -q -r /opt/backup check | tee -a ${BACKUP_LOG_FILE}
    ...
    RESTIC_PASSWORD=${ENV_NAME} restic -q -r /opt/backup backup --tag "${DUMP_NAME} ${BACKUP_ADDON_COMMIT_ID} ${BACKUP_TYPE}" ${APP_PATH} ~/wp_db_backup.sql | tee -a ${BACKUP_LOG_FILE}
    echo $(date) ${ENV_NAME} "Rotating snapshots by keeping the last ${BACKUP_COUNT}" | tee -a ${BACKUP_LOG_FILE}
    RESTIC_PASSWORD=${ENV_NAME} restic forget -q -r /opt/backup --keep-last ${BACKUP_COUNT} --prune | tee -a ${BACKUP_LOG_FILE}
    ...
    RESTIC_PASSWORD=${ENV_NAME} restic -q -r /opt/backup check --read-data-subset=1/10 | tee -a ${BACKUP_LOG_FILE}
